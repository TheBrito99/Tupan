...
import { HydraulicComponent, HydraulicConnection, EditorState, AnalysisData, DEFAULT_PARAMETERS } from './types';
import { Canvas } from './Canvas';
import { ComponentPalette } from './ComponentPalette';
import { PropertyPanel } from './PropertyPanel';
import { AnalysisPanel } from './AnalysisPanel';
import { analyzeHydraulicNetwork, validateHydraulicNetwork } from './hydraulicInteractions';
import styles from './HydraulicEditor.module.css';

interface EditorAction {
  type: 'ADD_COMPONENT' | 'UPDATE_COMPONENT' | 'DELETE_COMPONENT' |
        'ADD_CONNECTION' | 'DELETE_CONNECTION' |
        'SELECT_COMPONENT' | 'DESELECT_COMPONENT' |
        'START_DRAG' | 'STOP_DRAG' |
        'START_CONNECTION' | 'COMPLETE_CONNECTION' | 'CANCEL_CONNECTION' |
        'PAN' | 'ZOOM' | 'SET_MODE' | 'RUN_ANALYSIS';
  payload?: any;
}
...
function editorReducer(state: EditorState, action: EditorAction): EditorState {
  switch (action.type) {
    case 'ADD_COMPONENT': {
      const component: HydraulicComponent = {
    ...
    case 'ADD_CONNECTION': {
      const connection: HydraulicConnection = {
    ...
    case 'COMPLETE_CONNECTION': {
      if (state.drawingConnection && state.drawingConnection.from !== action.payload) {
        return {
          ...state,
          connections: [
            ...state.connections,
...
interface HydraulicEditorProps {
  onAnalysisComplete?: (data: AnalysisData) => void;
}

export const HydraulicEditor: React.FC<HydraulicEditorProps> = ({ onAnalysisComplete }) => {
  ...
  const handleComponentUpdate = (id: string, updates: Partial<HydraulicComponent>) => {
  ...
  const handleCanvasMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    if (state.draggingComponentId && e.buttons === 1) {
      const component = state.components.find(c => c.id === state.draggingComponentId);
      if (component) {
  ...
  const handleRunAnalysis = () => {
    const errors = validateHydraulicNetwork(state.components, state.connections);
    ...
    try {
      const results = analyzeHydraulicNetwork(state.components, state.connections);
      setAnalysisData(results);
      dispatch({ type: 'RUN_ANALYSIS', payload: results });
      onAnalysisComplete?.(results);
    } catch (error) {
      setValidationErrors([`Analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`]);
    }
  };
  ...
  return (
    <div className={styles.hydraulicEditor}>
      <div className={styles.toolbar}>
        <button onClick={() => dispatch({ type: 'SET_MODE', payload: 'select' })}
                className={state.mode === 'select' ? styles.active : ''}>
          Select
        </button>
        <button onClick={() => dispatch({ type: 'SET_MODE', payload: 'pan' })}
