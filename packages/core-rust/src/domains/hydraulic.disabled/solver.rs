...
//! - Hydraulic Resistance (R_h) [Pa·s/m³] ↔ Resistance (R) [Ω]
//! - Accumulator Volume (V) [m³] ↔ Capacitance (C) [F]
//!
//! Governing equation: G × P = Q
//! where G = 1/R_h (hydraulic conductance), P = pressure vector, Q = flow source vector
//!
//! Transient: (C/dt + G) × P_n = (C/dt) × P_{n-1} + Q

use super::components::{HydraulicComponent, analysis};
use nalgebra::{DMatrix, DVector};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;

/// Hydraulic system analyzer
/// Maps hydraulic components to MNA equation: G × P = Q
pub struct HydraulicAnalyzer {
    /// Component resistances [Pa·s/m³]
    resistances: HashMap<(usize, usize), f64>,
    /// Component capacitances [m³] (volumes)
    capacitances: HashMap<usize, f64>,
    /// Flow sources [m³/s] (pumps)
    flow_sources: HashMap<usize, f64>,
    /// Pressure sources [Pa]
    pressure_sources: HashMap<usize, f64>,
    /// Node count
    node_count: usize,
    /// Reference pressure [Pa] (tank pressure)
    tank_pressure: f64,
}

impl HydraulicAnalyzer {
    /// Create new hydraulic analyzer
    pub fn new() -> Self {
        HydraulicAnalyzer {
    ...
    /// Build hydraulic circuit from components and connections
    fn build_circuit(
        &mut self,
        components: &[(usize, usize, HydraulicComponent)],
        ...
        for (from, to, comp) in components {
            ...
            if let Some(r_h) = comp.get_resistance() {
                if r_h > 0.0 {
                    self.resistances.insert((*from, *to), r_h);
                }
            }
    ...
    pub fn solve_steady_state(
        ...
        // Build conductance matrix: G = 1/R_h for each resistance
        let mut g_matrix = DMatrix::<f64>::zeros(self.node_count, self.node_count);
        for ((from, to), r_h) in &self.resistances {
            let g = 1.0 / r_h;  // conductance = 1/resistance
        ...
        match g_matrix.try_inverse() {
            ...
            None => Err("Hydraulic system matrix is singular".to_string()),
        }
    }
    ...
    pub fn calculate_power(
        ...
        for ((from, to), r_h) in &self.resistances {
            if *from < pressures.len() && *to < pressures.len() {
                ...
                let flow = delta_p / r_h;
                let power = delta_p.abs() * flow.abs();  // Dissipated power
                total_power += power;
            }
        }
...
/// Hydraulic validation helper
pub struct HydraulicValidator;

impl HydraulicValidator {
    /// Validate hydraulic system for analysis
    pub fn validate_system(
        ...
        // Check for at least one pressure source or flow source
...
mod tests {
    ...
    fn test_analyzer_creation() {
        let analyzer = HydraulicAnalyzer::new();
        assert_eq!(analyzer.node_count(), 0);
    }

    #[test]
    fn test_simple_orifice_steady_state() {
        let components = vec![
        ...
        assert!((pressures[0] - 10000.0).abs() < 100.0);
    }
    ...
    fn test_series_resistances() {
        ...
        assert!((total_drop - 10000.0).abs() < 500.0);
    }
    ...
    #[test]
    fn test_accumulator_steady_state() {
        let components = vec![
            (0, 1, HydraulicComponent::Accumulator { volume: 0.01, precharge: 1e6 }),
        ];
        let flow_sources = vec![(0, 0.001)];

        let mut analyzer = HydraulicAnalyzer::new();
        let pressures = analyzer.solve_steady_state(&components, &flow_sources)
            .expect("Accumulator steady-state failed");
    ...
    #[test]
    fn test_pump_delivers_constant_flow() {
        let components = vec![
            (0, 1, HydraulicComponent::Orifice { resistance: 1e7 }),
        ];
        let flow_sources = vec![(0, 0.001)];
        ...
        let mut analyzer = HydraulicAnalyzer::new();
        let pressures = analyzer.solve_steady_state(&components, &flow_sources)
            .expect("Pump test failed");

        let expected_pressure = 1e7 * 0.001;
        assert!((pressures[0] - expected_pressure).abs() < 100.0);
    }

    #[test]
    fn test_cylinder_force_calculation() {
        let pressure = 2e6;
        let area = 0.01;
    ...
    #[test]
    fn test_motor_torque_calculation() {
        let pressure = 2e6;
        let displacement = 50.0;
        let torque = analysis::motor_torque(pressure, displacement);
        assert!(torque > 0.0);
        assert!((torque - 159.15).abs() < 10.0);
    }

    #[test]
    fn test_orifice_flow_calculation() {
        let area = 1e-4;
        let pressure_drop = 1e6;
        let density = 860.0;

        let flow = analysis::orifice_flow(area, pressure_drop, density)
            .expect("Orifice flow calc failed");

        assert!(flow > 0.0);
        assert!(flow > 0.005 && flow < 0.02);
    }

    #[test]
    fn test_accumulator_charge_time() {
        let volume = 0.01;
        let flow_rate = 0.001;
        let precharge = 1e6;
        let target = 5e6;

        let time = analysis::accumulator_charge_time(volume, flow_rate, precharge, target)
            .expect("Charge time calc failed");

        assert!(time > 0.0);
        assert!(time > 10.0 && time < 30.0);
    }

    #[test]
    fn test_transient_accumulator_response() {
        let components = vec![
            (0, 1, HydraulicComponent::Accumulator { volume: 0.01, precharge: 1e6 }),
        ];
        let flow_sources = vec![(0, 0.001)];
        ...
        if pressures.len() > 1 {
            assert!(pressures[pressures.len()-1][0] >= pressures[0][0]);
        }
    }

    #[test]
    fn test_power_dissipation_in_orifice() {
        let components = vec![
            (0, 1, HydraulicComponent::Orifice { resistance: 1e7 }),
        ];
        let flow_sources = vec![(0, 0.001)];
        ...
        assert!((power - 10.0).abs() < 1.0);
    }

    #[test]
    fn test_invalid_system() {
        let components = vec![];
        assert!(HydraulicValidator::validate_system(&components).is_err());
    }
    ...
    #[test]
    fn test_pressure_relief_valve() {
        let valve = HydraulicComponent::PressureReliefValve {
            cracking_pressure: 3e6,
            flow_capacity: 0.01,
        };
